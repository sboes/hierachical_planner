# create 10 shaply environments from easy to easy-medium, medium, medium-hard, hard, very-hard, extreme, and impossible
...
def create_cross_shape_scene() -> Tuple[Dict[str, Polygon], Tuple[Tuple[float, float], Tuple[float, float]]]:
    """Kreuzförmiges Hindernis in der Mitte des Raums."""
    wall1 = LineString([(6, 18), (6, 8)]).buffer(1.0)
    wall2 = LineString([(16, 18), (16, 8)]).buffer(1.0)
    wall3 = LineString([(6, 12), (16, 12)]).buffer(1.0)
...
def create_corridor_scene() -> Tuple[Dict[str, Polygon], Tuple[Tuple[float, float], Tuple[float, float]]]:
    ...
    left = Polygon([(3, 0), (5, 0), (5, 18), (3, 18.0)])
...
def create_multiple_obstacles_scene() -> Tuple[Dict[str, Polygon], Tuple[Tuple[float, float], Tuple[float, float]]]:
    ...
    obstacles = {
        f"obstacle_{i}": Polygon([(i * 2, 2), (i * 2 + 1, 2), (i * 2 + 1, 4), (i * 2, 4)])
        for i in range(10)
    }
    scene = obstacles
    limits = ((0, 22), (0, 22))
...
def get_all_scenes() -> List[Tuple[str, Tuple[Dict[str, Polygon], Tuple[Tuple[float, float], Tuple[float, float]]]]]:
    ...
    return [
        ...
        ("Cross-Shape", create_cross_shape_scene()),
